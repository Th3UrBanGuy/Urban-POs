...
import { verifyMasterKey } from '@/app/actions';

const masterKeyUnlockSchema = z.object({
...
export default function AccessKeysPage() {
  ...
  const unlockForm = useForm<z.infer<typeof masterKeyUnlockSchema>>({
    resolver: zodResolver(masterKeyUnlockSchema),
  ...
  const keyForm = useForm<z.infer<typeof keyFormSchema>>({
    ...
    defaultValues: { tagName: '', key: '', isMasterKey: false, permissions: ['pos'] },
  });

  // ── Unlock this management page ─────────────────────────────────────────────

  const onUnlockSubmit = async (values: z.infer<typeof masterKeyUnlockSchema>) => {
    const result = await verifyMasterKey(values.masterKey);
    if (result.success) {
      setIsUnlocked(true);
      toast({ title: 'Access Granted', description: 'You can now manage access keys.' });
    } else {
      unlockForm.setError('masterKey', { type: 'manual', message: result.message });
      toast({ variant: 'destructive', title: 'Access Denied', description: result.message });
    }
  };


  // ── Create / Delete access key ───────────────────────────────────────────────
  ...
  const handleAddKey = (values: z.infer<typeof keyFormSchema>) => {
    startTransition(async () => {
      ...
      try {
        ...
        toast({ title: 'Key Created', description: `New key "${values.key}" has been created.` });
        keyForm.reset();
      } catch {
      ...
      <div className="flex items-center justify-center h-full">
        <Card className="w-full max-w-sm">
          <CardHeader>
            ...
            <CardDescription>
              Enter a master key to manage all access keys. This request is validated server-side.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <Form {...unlockForm}>
              <form onSubmit={unlockForm.handleSubmit(onUnlockSubmit)} className="space-y-4">
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormControl>
                        <Input type="password" placeholder="Enter Master Key" {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
  ...
  const KeyFormFields = () => {
    const isMaster = keyForm.watch('isMasterKey');
    return (
      <>
        <FormField control={keyForm.control} name="tagName" render={({ field }) => (
          <FormItem>
            <FormLabel>Tag Name</FormLabel>
            <FormControl><Input placeholder="e.g., Main Cashier" {...field} /></FormControl>
            <FormMessage />
          </FormItem>
        )} />
        <FormField control={keyForm.control} name="key" render={({ field }) => (
          <FormItem>
            ...
            <FormControl><Input placeholder="e.g., CASHIER01" {...field} /></FormControl>
            <FormMessage />
          </FormItem>
        )} />
        <FormField control={keyForm.control} name="isMasterKey" render={({ field }) => (
          <FormItem className="flex flex-row items-center justify-between rounded-lg border p-3 shadow-sm">
            <div className="space-y-0.5">
              <FormLabel>Master Key</FormLabel>
              <p className="text-xs text-muted-foreground">Grants full access to all features.</p>
            </div>
            <FormControl>
        ...
        {!isMaster && (
          <FormField control={keyForm.control} name="permissions" render={() => (
            <FormItem>
              <div className="mb-4">
                <FormLabel className="text-base">Permissions</FormLabel>
                <p className="text-sm text-muted-foreground">Select which pages this key can access.</p>
              </div>
              <div className="space-y-2">
                {PERMISSIONS.map((item) => (
                  <FormField key={item.id} control={keyForm.control} name="permissions" render={({ field }) => (
                  ...
                  {accessKeys?.map((accessKey) => (
                    <TableRow key={accessKey.id}>
                      ...
                      <TableCell className="text-right">
                        ...
                        <Button variant="ghost" size="icon" onClick={() => handleDeleteKey(accessKey.id)}>
          ...
          <Card className="border-amber-200 dark:border-amber-800">
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                ...
                Master Key (Environment-Controlled)
              </CardTitle>
              <CardDescription>
                The master key is stored as a secure server-side environment variable (<code className="text-xs bg-muted px-1 py-0.5 rounded">MASTER_KEY</code>).
                It is <strong>never</strong> compiled into browser code.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-3 text-sm text-muted-foreground">
              <div className="p-3 bg-muted rounded-lg space-y-1">
                <p className="font-medium text-foreground">To change the master key:</p>
                <ul className="ml-4 list-disc space-y-1">
                  <li><strong>Development:</strong> Update <code className="text-xs bg-background px-1 rounded">MASTER_KEY</code> in your <code className="text-xs bg-background px-1 rounded">.env</code> file and restart the server.</li>
                  <li><strong>Firebase App Hosting:</strong> Go to Firebase Console → App Hosting → your backend → Environment Variables → update <code className="text-xs bg-background px-1 rounded">MASTER_KEY</code>.</li>
                </ul>
              </div>
              <p className="text-xs">
                Custom access keys (above) are managed in Firestore and can be created, assigned permissions, and deleted at any time without restarting the server.
              </p>
            </CardContent>
          </Card>
        </div>
